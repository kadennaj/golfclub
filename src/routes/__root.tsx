...
const SOCIAL_IMAGE_URL = siteUrl("/og/richmond-hill-golf-club.jpg");
...
function NotFoundComponent() {
  useEffect(() => {
    document.title = "Page Not Found | Richmond Hill Golf Club";
  }, []);
...
function ErrorComponent({ error, reset }: { error: Error; reset: () => void }) {
  useEffect(() => {
    document.title = "Page Error | Richmond Hill Golf Club";
  }, []);
...
function RootComponent() {
  return (
    <>
      <a
        href="#main-content"
        className="fixed left-4 top-4 z-[100] -translate-y-24 bg-cream px-5 py-3 text-xs uppercase tracking-[0.2em] text-charcoal transition-transform focus:translate-y-0"
      >
